...
class SemSegTester(TesterBase):
    def test(self):
        ...
        # memory_allocated_meter = AverageMeter()
        # memory_reserved_meter = AverageMeter()
        # peak_memory_allocated_meter = AverageMeter()
        peak_memory_reserved_meter = AverageMeter()
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # end_memory_allocated = torch.cuda.memory_allocated() / (1024 ** 3)
            # end_memory_reserved = torch.cuda.memory_reserved() / (1024 ** 3)
            # peak_memory_allocated = torch.cuda.max_memory_allocated() / (1024 ** 3)
            peak_memory_reserved = torch.cuda.max_memory_reserved() / (1024 ** 3)

            # memory_allocated_meter.update(end_memory_allocated)
            # memory_reserved_meter.update(end_memory_reserved)
            # peak_memory_allocated_meter.update(peak_memory_allocated)
            peak_memory_reserved_meter.update(peak_memory_reserved)
            ...
            logger.info(
                "Test: {} [{}/{}]-{} "
                "Batch {batch_time.val:.3f} ms ({batch_time.avg:.3f}) ms "
                "Memory: {peak_mem_res.val:.1f} GB ({peak_mem_res.avg:.1f} GB) "
                "Accuracy {acc:.4f} ({m_acc:.4f}) "
                "mIoU {iou:.4f} ({m_iou:.4f}) "
                # "Memory Allocated: {mem_alloc.val:.1f} GB ({mem_alloc.avg:.1f} GB) "
                # "Memory Reserved: {mem_res.val:.1f} GB ({mem_res.avg:.1f} GB) "
                # "Peak Allocated: {peak_mem_alloc.val:.1f} GB ({peak_mem_alloc.avg:.1f} GB) "
                ...
                .format(
                    ...
                    iou=iou,
                    m_iou=m_iou,
                    # mem_alloc=memory_allocated_meter,
                    # mem_res=memory_reserved_meter,
                    # peak_mem_alloc=peak_memory_allocated_meter,
                    peak_mem_res=peak_memory_reserved_meter,
                )
            )
